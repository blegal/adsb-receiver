...
{
    const int bComplex = cbuffer.size();      // Nombre de complexes
    const int nComplex = bComplex - coverage; // Nombre de complexes a recuperer
    const int nBytes   = 2 * nComplex;        // Nombre de donnée à lire

    //
    // On commence par realiser l'overlap entre les fenetres
    // - On décale (coverage) complexes de la fin au début
    //
    const int nOffset  = cbuffer.size() - coverage;
    for(int i = 0; i < coverage; i += 1)
        cbuffer[i] = cbuffer[nOffset + i];

    //
    // On remplit l'espace (coverage => bComplex) par des 1
    //
    for(int i = coverage; i < bComplex; i += 1)
        cbuffer[i] = 0;
